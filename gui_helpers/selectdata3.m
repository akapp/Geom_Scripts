...
%           'Ignore' , 'Pointer', 'SelectionMode', 'Flip'
%
%  'Action' - {'list', 'delete'}
...
while selectionflag
  ...
  switch params.SelectionMode
    case 'closest'
      ...
      [pointslist,xselect,yselect,zselect] = closestpoint(xyz,xdata,ydata,zdata,dx,dy);
      nsel = 1;
    ...
    case 'lasso'
      ...
      % Confirm mouse down before starting lasso
      key = 1;
      while key~=0
...
function CPmotion(src,evnt) %#ok
  ...
  % what point was closest?
  [pointslist,xselect,yselect,zselect] = closestpoint(xyz,xdata,ydata,dx,dy);
  nsel = 1;
...
function lassomotion(src,evnt) %#ok
  ...
  set(selecthandle,'xdata',xv,'ydata',yv,'zdata',zv)
  
  % did we enclose any new points?
  [pointslist,xselect,yselect,zselect,nsel] = testpoly3(xv,yv,zv,xdata,ydata,zdata);
...
%                  subfunctions
% ================================================
function [pl,xsel,ysel,zsel,nsel] = testpoly(xv,yv,zv,xdata,ydata,zdata)
...
%                  subfunction
% ================================================
function [pointslist,xselect,yselect,zselect] = closestpoint(xyz,xdata,ydata,zdata,dx,dy)
